...
function hasActiveEvents(org, prefix) {
  ...
  // Always show City Council, Borough Presidents, and NYPD precincts (meetings may be scheduled later)
  if (prefix.startsWith("city-council") || prefix.startsWith("borough-presidents") || prefix.startsWith("city-agencies.nypd")) {
    return true;
  }
...
function getLeafKeys(org, prefix) {
  ...
  if (org.children) {
    for (const [key, child] of Object.entries(org.children)) {
      const fullKey = prefix ? `${prefix}.${key}` : key;
      // Skip inactive orgs (but always include city-council, borough-presidents, and nypd precincts)
      if (activeOrgs.size > 0 && !fullKey.startsWith("city-council") && !fullKey.startsWith("borough-presidents") && !fullKey.startsWith("city-agencies.nypd") && !hasActiveEvents(child, fullKey)) {
        continue;
      }
      keys.push(...getLeafKeys(child, fullKey));
    }
  } else {
    // Only add leaf if it's active, or if it's city-council/borough-presidents/nypd, or if we haven't loaded activeOrgs yet
    if (activeOrgs.size === 0 || prefix.startsWith("city-council") || prefix.startsWith("borough-presidents") || prefix.startsWith("city-agencies.nypd") || activeOrgs.has(prefix)) {
      keys.push(prefix);
    }
  }
  return keys;
