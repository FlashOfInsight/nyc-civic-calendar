...
async function scrapeMTA() {
  ...
  try {
    ...
    // Find all meeting info - both links and plain text
    ...
    $("a").each((_, elem) => {
      ...
      if (dateMatch && href.includes("board-and-committee-meetings")) {
        processMeeting(dateMatch, href);
      }
    });

    // Also check for plain text entries (2026 meetings may not have links yet)
    $("p, li, div, span").each((_, elem) => {
      const text = $(elem).text().trim();

      // Match the full pattern including "MTA Committee" to avoid false positives
      const fullMatch = text.match(/(\w+)\s+(\d+)\s+and\s+(\d+),\s+(\d{4}):\s*MTA\s+(Committee|Board)/i);

      if (fullMatch) {
        const dateMatch = text.match(/(\w+)\s+(\d+)\s+and\s+(\d+),\s+(\d{4})/i);
        if (dateMatch) {
          // Check if we already have this meeting (from a link)
          const year = parseInt(dateMatch[4]);
          const month = parseMonth(dateMatch[1]);
          const existingId = `mta-board-${year}-${month + 1}-${parseInt(dateMatch[3])}`;

          if (!meetings.find(m => m.id === existingId)) {
            processMeeting(dateMatch, "https://www.mta.info/transparency/board-and-committee-meetings");
          }
        }
      }
    });

    function processMeeting(dateMatch, href) {
