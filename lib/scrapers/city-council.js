...
function fetchAPI(path) {
  ...
  const url = `${API_BASE}${path}${separator}Token=${encodeURIComponent(LEGISTAR_TOKEN)}`;

  console.log(`Legistar API request: ${API_BASE}${path}${separator}Token=[REDACTED]`);

  return new Promise((resolve, reject) => {
    https.get(url, { headers: { Accept: "application/json", "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36" } }, (res) => {
      if (res.statusCode !== 200) {
        let body = "";
        res.on("data", chunk => body += chunk);
        res.on("end", () => {
          console.error(`Legistar API ${res.statusCode}: ${body.slice(0, 200)}`);
...
async function scrapeCityCouncil() {
  ...
  const events = await fetchAPI(
    `/events?$filter=${encodeURIComponent(filter)}&$orderby=${encodeURIComponent('EventDate desc')}&$top=500`
  );
  ...
  for (const event of events) {
    ...
    meetings.push({
      ...
      description: "Livestream: https://council.nyc.gov/livestream/" + (event.EventComment ? "\n\n" + event.EventComment : ""),
      url: url,
