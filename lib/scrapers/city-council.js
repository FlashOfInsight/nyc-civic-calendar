...
const { fetchHTML } = require("../scraper-utils");
...
/**
 * Fetch URL and return response body as string. Legistar needs an explicit
 * Accept header (it varies its response format without one) and a browser
 * User-Agent (its default one gets blocked).
 */
...
async function scrapeViaAPI() {
  ...
  for (const event of events) {
    ...
    // Skip deferred, recessed, or cancelled meetings
    const agendaStatus = (event.EventAgendaStatusName || "").toLowerCase();
    if (agendaStatus === "deferred" || agendaStatus === "recessed" || agendaStatus === "postponed" || agendaStatus === "cancelled") {
      deferredCount++;
      continue;
    }
  ...
  console.log(`City Council: ${meetings.length} meetings via API (${deferredCount} deferred/skipped)`);
  return meetings;
}
...
async function scrapeViaHTML() {
  ...
  $("tr").each((_, row) => {
    ...
    const status = $(cells[2]).text().trim().toLowerCase();
    const timeStr = $(cells[3]).text().trim();
    const location = $(cells[4]).text().trim();

    // Skip deferred, recessed, or postponed meetings
    if (status === "deferred" || status === "recessed" || status === "postponed" || status === "cancelled") return;
