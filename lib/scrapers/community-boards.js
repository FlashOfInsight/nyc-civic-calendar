...
const { fetchHTML: fetchHTMLBase, parseMonth, formatDate, getNthWeekday } = require("../scraper-utils");

/**
 * Fetch HTML from URL with redirect handling. Community board sites are
 * more likely to block a generic scraper User-Agent, so this uses a
 * browser-like one instead of the shared default.
 */
function fetchHTML(url, opts = {}) {
  return fetchHTMLBase(url, {
...
const MANHATTAN_BOARDS = [
  { id: 1, name: "CB1 Manhattan", neighborhoods: "Financial District, Battery Park City, Tribeca", type: "cb1m-rest", url: "https://manhattancb1.cityofnewyork.us/meetings/calendar/" },
  ...
  { id: 8, name: "CB8 Manhattan", neighborhoods: "Upper East Side, Roosevelt Island", type: "cb8m-rest", url: "https://www.cb8m.com" },
  { id: 9, name: "CB9 Manhattan", neighborhoods: "Morningside Heights, Hamilton Heights", type: "cb9m-gcal", url: "https://www.cb9m.org" },
...
const QUEENS_BOARDS = [
  ...
  { id: 2, name: "CB2 Queens", neighborhoods: "Woodside, Sunnyside, Long Island City", type: "queens-cb2-gcal", url: "https://www.nyc.gov/site/queenscb2/calendar/calendar.page" },
  ...
  { id: 4, name: "CB4 Queens", neighborhoods: "Corona, Elmhurst", type: "queens-cb4-csv", url: "https://www.nyc.gov/site/queenscb4/calendar/calendar.page" },
  { id: 5, name: "CB5 Queens", neighborhoods: "Ridgewood, Glendale, Maspeth, Middle Village", type: "queens-cb5-js", url: "https://www.nyc.gov/site/queenscb5/calendar/calendar.page" },
  { id: 6, name: "CB6 Queens", neighborhoods: "Forest Hills, Rego Park", type: "queens-cb6-gcal", url: "https://www.nyc.gov/site/queenscb6/calendar/calendar.page" },
  { id: 7, name: "CB7 Queens", neighborhoods: "Flushing, Whitestone, College Point", type: "queens-cb7-static", url: "https://www.nyc.gov/site/queenscb7/meetings/meetings.page" },
  ...
  { id: 13, name: "CB13 Queens", neighborhoods: "Queens Village, Cambria Heights, Laurelton, Rosedale", type: "queens-cb13-gcal", url: "https://www.nyc.gov/site/queenscb13/calendar/calendar.page" },
...
const BRONX_BOARDS = [
  ...
  { id: 4, name: "CB4 Bronx", neighborhoods: "Highbridge, Concourse, Mount Eden", type: "bronx-cb4-calendar", url: "https://www.nyc.gov/site/bronxcb4/calendar/calendar.page" },
  ...
  { id: 9, name: "CB9 Bronx", neighborhoods: "Soundview, Parkchester, Castle Hill", type: "bronx-cb9-calendar", url: "https://www.nyc.gov/site/bronxcb9/calendar/calendar.page" },
  ...
  { id: 12, name: "CB12 Bronx", neighborhoods: "Williamsbridge, Wakefield, Woodlawn", type: "bronx-cb12-generated", url: "https://www.nyc.gov/site/bronxcb12/calendar/calendar.page" }
];
...
const BROOKLYN_BOARDS = [
  ...
  { id: 3, name: "CB3 Brooklyn", neighborhoods: "Bedford-Stuyvesant", type: "cb3brooklyn-gcal", url: "https://www.nyc.gov/site/brooklyncb3/calendar/calendar.page" },
  { id: 4, name: "CB4 Brooklyn", neighborhoods: "Bushwick", type: "brooklyn-cb4-js", url: "https://www.nyc.gov/site/brooklyncb4/calendar/calendar.page" },
  ...
  { id: 7, name: "CB7 Brooklyn", neighborhoods: "Sunset Park, Windsor Terrace", type: "cbbrooklyn", url: "https://cbbrooklyn.cityofnewyork.us/cb7/events/" },
  ...
  { id: 9, name: "CB9 Brooklyn", neighborhoods: "Crown Heights, Prospect Lefferts Gardens", type: "brooklyn-cb9-csv", url: "https://www.nyc.gov/site/brooklyncb9/calendar/calendar.page" },
  ...
  { id: 18, name: "CB18 Brooklyn", neighborhoods: "Canarsie, Mill Basin, Flatlands, Marine Park", type: "brooklyn-cb18-js", url: "https://www.nyc.gov/site/brooklyncb18/meetings/calendar.page" }
];
...
/**
 * Parse a Google Calendar public ICS feed into meeting objects.
 * Used by boards that embed a Google Calendar on their website.
 * @param {string[]} calIds - list of GCal IDs (without @group.calendar.google.com)
 * @param {number} boardId - community board number
 * @param {string} borough - borough slug
 * @param {string} boardUrl - display URL for the board
 * @param {RegExp|null} titleFilter - optional regex; if set, only matching events are kept
 */
const HOLIDAY_RE = /\b(christmas|new year|martin luther|presidents.?day|memorial day|independence day|labor day|thanksgiving|juneteenth|columbus day|veterans day|election day|holiday)\b/i;

async function scrapeGCalICS(calIds, boardId, borough, boardUrl, titleFilter = null, titleExclude = null) {
  const meetings = [];
  const seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);

  const boroughAbbrevs = { "manhattan": "M", "bronx": "Bx", "brooklyn": "Bk", "queens": "Qn", "staten-island": "SI" };
  const abbrev = boroughAbbrevs[borough] || "M";

  for (const calId of calIds) {
    // Accept bare IDs ("abc123"), full group addresses ("abc123@group.calendar.google.com"),
    // or Google Workspace emails ("bk03@cb.nyc.gov")
    const fullId = calId.includes("@") ? calId : calId + "@group.calendar.google.com";
    const icsUrl = `https://calendar.google.com/calendar/ical/${encodeURIComponent(fullId)}/public/basic.ics`;
    try {
      const icsText = await fetchHTML(icsUrl);
      const content = icsText.replace(/\r\n/g, "\n").replace(/\n[ \t]/g, "");
      for (const block of content.split("BEGIN:VEVENT").slice(1)) {
        ...
        if (titleFilter && !titleFilter.test(summary)) continue;
        if (titleExclude && titleExclude.test(summary)) continue;

        const locationMatch = block.match(/^LOCATION:(.+)$/m);
        const location = locationMatch ? locationMatch[1].trim() : `Brooklyn CB${boardId} Office`;

        const dateStr = formatDate(year, month, day);
        const meetingName = `CB${abbrev}${boardId}: ${summary}`;
        const key = `${dateStr}|${meetingName}`;
        if (seen.has(key)) continue;
        seen.add(key);

        meetings.push(createMeeting(boardId, meetingName, dateStr, time, location, boardUrl, borough));
      }
    } catch (err) {
      // Calendar not accessible or private — skip silently
    }
  }
...
/**
 * Scrape CB9 Manhattan from Google Calendar (g4b54u7hbpp1b6p63gp0n97448@group.calendar.google.com).
 * The calendar mixes board meetings with holidays — exclude the holidays.
 * Board posts meetings sporadically; falls back to generated schedule when < 2 found.
 */
async function scrapeCB9ManhattanGCal(board) {
  return scrapeGCalICS(
    ["g4b54u7hbpp1b6p63gp0n97448"],
    9, "manhattan", board.url,
    null,        // no title whitelist
    HOLIDAY_RE   // exclude holidays
  );
}

/**
 * Scrape Brooklyn CB3 from Google Calendar (bk03@cb.nyc.gov).
 * The nyc.gov page embeds this calendar via iframe.
 */
async function scrapeCB3BrooklynGCal(board) {
  return scrapeGCalICS(["bk03@cb.nyc.gov"], 3, "brooklyn", board.url);
}

/**
 * Scrape Queens CB2 from 14 committee Google Calendars embedded on the calendar page.
 * Calendar IDs extracted from: https://www.nyc.gov/site/queenscb2/calendar/calendar.page
 */
async function scrapeQB2MultiGCal(board) {
  const CAL_IDS = [
    "51lqdhce33l9c67k1i645dejqs", "g7ocnlni219nlt7gob8tc470oc",
    "20gborbt6oksv70e5j0a0hoebk", "950ocboi9ds9m5kfe69mg59db0",
    "ij1u8hpq0bjh3v099mne418il4", "28ru4snbijf1ipteprvph0s0c0",
    "69eed2bn65b21e0svqfc7t4eog", "edl3uca9ngn2qvc2uj6leeatrk",
    "7koql1004fad24af7kgn5n7dhk", "ti0hvl15nkmqn6osd8vkaidbog",
    "1lvipg1herl0rqt22erj2qamek", "t613gs3ukqiab5hgeibu27rfjs",
    "3nu7j1jcp8fee6vldjgg8r7c94", "i93d0fg2o6033qhs8tfmqs3ng0"
  ];
  return scrapeGCalICS(CAL_IDS, 2, "queens", board.url);
}
...
/**
 * Scrape Brooklyn CB18 (Canarsie/Mill Basin/Flatlands) from calendar-events.js.
 * Format: calEvents[n] = "M/D/YYYY|<span>...<strong>Name</strong>...<hr/>...<strong>Name2</strong>...</span>||0|na|"
 * One date entry can contain multiple meetings separated by <hr>. CANCELED entries are skipped.
 */
async function scrapeCB18BrooklynJS(board) {
  const JS_URL = "https://www.nyc.gov/assets/brooklyncb18/js/calendar-events.js";
  const LOCATION = "1097 Bergen Avenue, Brooklyn, NY 11234";
  const meetings = [], seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);

  try {
    const js = await fetchHTML(JS_URL);

    for (const [, ds, rawHtml] of js.matchAll(/calEvents\[calEvents\.length\]\s*=\s*"(\d+\/\d+\/\d+)\|(.+)\|\|0\|na\|";/g)) {
      ...
      // Split on <hr> — each section is a separate committee meeting
      for (const section of html.split(/<hr\s*\/?>/i)) {
        // Extract all consecutive <strong> texts before date/time info
        const nameParts = [];
        for (const [, inner] of section.matchAll(/<strong>(.*?)<\/strong>/gi)) {
          // Take text before any <br — stop at "When:", day names, etc.
          const part = inner.split(/<br/i)[0]
            .replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").replace(/&rsquo;|&#8217;/g, "'")
            .replace(/&nbsp;/g, " ").trim();
          if (!part || /^When:|^\w+day,|^Time:|^Place:/i.test(part)) break;
          nameParts.push(part);
        }
        const name = nameParts.join(" ").replace(/\s+/g, " ").trim();
        if (!name || !isValidMeetingTitle(name)) continue;
        if (/CANCEL/i.test(name)) continue;

        // Time: "Starting at 7PM", "6:30 pm", "3:30 pm", etc.
        const timeMatch = section.match(/(?:starting at\s+)?(\d{1,2}(?::\d{2})?\s*[ap]m)/i);
        const time = timeMatch ? parseTime(timeMatch[1]) || "19:00" : "19:00";

        const meetingName = `CB18: ${name}`;
        const key = `${dateStr}|${meetingName}`;
        if (seen.has(key)) continue; seen.add(key);

        meetings.push(createMeeting(18, meetingName, dateStr, time, LOCATION, board.url, "brooklyn"));
      }
    }
  } catch (err) { console.error("Error scraping BK CB18 JS:", err.message); }
  return meetings.filter(Boolean);
}
...
/**
 * Scrape Brooklyn CB4 from /assets/brooklyncb4/js/calendar-events.js.
 * Events are a JS array: calEvents[n] = 'M/D/YYYY|<HTML>||0|na|';
 * Board meetings include a Zoom registration link; holidays/observances do not.
 */
async function scrapeCB4BrooklynJS(board) {
  const JS_URL = "https://www.nyc.gov/assets/brooklyncb4/js/calendar-events.js";
  const meetings = [];
  const seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);
  ...
  try {
    ...
    for (const [, dateStr, html] of js.matchAll(/calEvents\[calEvents\.length\]\s*=\s*'(\d+\/\d+\/\d+)\|(.+)\|\|0\|na\|';/g)) {
      // Only board meetings have Zoom links
      if (!html.includes("zoom.us")) continue;
      // Skip if cancelled
      if (/CANCEL/i.test(html.slice(0, 80))) continue;

      const [m, d, y] = dateStr.split("/").map(Number);
      const meetingDate = new Date(y, m - 1, d);
      if (meetingDate < now || meetingDate > sixMonthsLater) continue;

      // Committee name: find <strong> inside a <p> tag (board meetings use <p><strong>Name</strong><br/>time)
      // This skips event banners that appear before the actual meeting in the same entry
      const strongMatch = html.match(/<p>\s*<strong>(.*?)<\/strong>/i) || html.match(/<strong>(.*?)<\/strong>/i);
      if (!strongMatch) continue;
      const name = strongMatch[1].replace(/<[^>]+>/g, "").replace(/&nbsp;|&amp;|&rsquo;/g, " ").trim();
      if (!name || name.length < 3 || !isValidMeetingTitle(name)) continue;

      // Time: search after the matched committee name to avoid picking up earlier event times
      const afterName = html.slice(strongMatch.index + strongMatch[0].length);
      const timeMatch = afterName.match(/(\d{1,2}(?::\d{2})?\s*[ap]m)/i);
      const time = timeMatch ? parseTime(timeMatch[1]) || "18:00" : "18:00";

      const dateStr2 = formatDate(y, m - 1, d);
      const meetingName = `CB4: ${name}`;
      const key = `${dateStr2}|${meetingName}`;
      if (seen.has(key)) continue;
      seen.add(key);

      meetings.push(createMeeting(4, meetingName, dateStr2, time, "127 Linden Blvd, Brooklyn, NY 11226", board.url, "brooklyn"));
    }
  } catch (err) {
    console.error("Error scraping BK CB4 JS calendar:", err.message);
  }
...
/**
 * Parse a CSV row respecting double-quoted fields (RFC 4180).
 */
function parseCSVRow(line) {
  const fields = [];
  let i = 0;
  while (i <= line.length) {
    if (i === line.length || line[i] === ",") { fields.push(""); i++; continue; }
    if (line[i] === '"') {
      let end = i + 1;
      ...
      fields.push(line.slice(i + 1, end - 1).replace(/""/g, '"'));
      i = end + 1;
    } else {
...
/**
 * Scrape Brooklyn CB9 from /assets/brooklyncb9/data/calendar.csv.
 * Format: date,start,end,name,about,location — rows with an empty start time are holidays.
 */
async function scrapeCB9BrooklynCSV(board) {
  const CSV_URL = "https://www.nyc.gov/assets/brooklyncb9/data/calendar.csv";
  const meetings = [];
  const seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);
  ...
  try {
    const csv = await fetchHTML(CSV_URL);
    const lines = csv.replace(/\r\n/g, "\n").split("\n");

    for (let i = 1; i < lines.length; i++) {  // skip header
      const line = lines[i].trim();
      if (!line) continue;
      const [dateStr, startTime, , name, , location] = parseCSVRow(line);
      if (!startTime) continue;  // holidays have no start time
      ...
      if (!isValidMeetingTitle(cleanName)) continue;

      const time = parseTime(startTime) || "19:00";
      const cleanLocation = location ? location.replace(/""/g, '"').trim() : "890 Nostrand Ave, Brooklyn, NY 11225";
      const dateStr2 = formatDate(y, m - 1, d);
      const meetingName = `CB9: ${cleanName}`;
      const key = `${dateStr2}|${meetingName}`;
      if (seen.has(key)) continue;
      seen.add(key);

      meetings.push(createMeeting(9, meetingName, dateStr2, time, cleanLocation || "890 Nostrand Ave, Brooklyn, NY 11225", board.url, "brooklyn"));
    }
  } catch (err) {
    console.error("Error scraping BK CB9 CSV calendar:", err.message);
  }
    ...
    try {
      switch (board.type) {
        ...
        case "cb8brooklyn":
          ...
          break;
        case "cb3brooklyn-gcal":
          meetings = await scrapeCB3BrooklynGCal(board);
          break;
        ...
        case "brooklyn-cb1-pdf":
          ...
          break;
        case "brooklyn-cb4-js":
          meetings = await scrapeCB4BrooklynJS(board);
          break;
        case "brooklyn-cb5-generated":
          meetings = generateBK5BrooklynMeetings(board);
          break;
        case "brooklyn-cb9-csv":
          meetings = await scrapeCB9BrooklynCSV(board);
          break;
        case "brooklyn-cb18-js":
          meetings = await scrapeCB18BrooklynJS(board);
          break;
        case "cb11brooklyn":
      ...
      if (meetings.length < 2) {
        ...
        // CB8/CB9 Brooklyn: confirmed schedules from live pages
        const cb8Committees = board.id === 9 ? [
          { name: "Full Board Meeting",    weekday: 2, nth: 4, time: "18:30" }, // 4th Tuesday
          { name: "Executive Committee",   weekday: 2, nth: 3, time: "18:30" }  // 3rd Tuesday
        ] : board.id === 8 ? [
...
/**
 * Scrape CB1 Manhattan (Financial District, Battery Park City, Tribeca) via Tribe REST API.
 * Site: manhattancb1.cityofnewyork.us — a separate WordPress install from nyc.gov.
 * Filters out holiday/office-closed entries (no venue or title contains "Office Closed").
 */
async function scrapeCB1ManhattanREST(board) {
  const API_BASE = "https://manhattancb1.cityofnewyork.us/wp-json/tribe/events/v1/events";
  const meetings = [], seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);
  const startDate = formatDate(now.getFullYear(), now.getMonth(), now.getDate());
  ...
  try {
    ...
    while (page <= totalPages && page <= 5) {
      ...
      for (const event of data.events || []) {
        ...
        if (!title || /office closed|holiday/i.test(title)) continue;
        ...
        meetings.push(createMeeting(1, title, dateStr, `${d.hour}:${d.minutes}`,
          location || "1 Centre Street, New York, NY", board.url));
      }
      page++;
    }
  } catch (err) { console.error("Error scraping CB1 Manhattan REST:", err.message); }
  return meetings.filter(Boolean);
}

/**
 * Scrape CB8 Manhattan (Upper East Side) via the Tribe Events REST API on cb8m.com.
 * The site was Cloudflare-blocked at the homepage but the WP REST API responds normally.
 * Skip events with CANCELED/CANCEL prefix.
 */
async function scrapeCB8ManhattanREST(board) {
  const API_BASE = "https://www.cb8m.com/wp-json/tribe/events/v1/events";
  const meetings = [];
  const seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);
  ...
  try {
    ...
    while (page <= totalPages && page <= 5) {
      ...
      for (const event of data.events || []) {
        ...
        const dateStr = formatDate(year, month, day);
        const key = `${dateStr}|${rawTitle}`;
        if (seen.has(key)) continue; seen.add(key);

        meetings.push(createMeeting(8, rawTitle, dateStr, `${d.hour}:${d.minutes}`,
          location || "505 Park Avenue, New York, NY", board.url));
      }
      page++;
    }
  } catch (err) {
    console.error("Error scraping CB8 Manhattan REST:", err.message);
  }
    ...
    try {
      switch (board.type) {
        ...
        case "nyc-gov-cb1":
          ...
          break;
        case "cb1m-rest":
          meetings = await scrapeCB1ManhattanREST(board);
          break;
        case "nyc-gov-cb3":
        ...
        case "cb8m":
          ...
          break;
        case "cb8m-rest":
          meetings = await scrapeCB8ManhattanREST(board);
          break;
        case "cb9m":
          meetings = generateCB9Meetings();
          break;
        case "cb9m-gcal":
          meetings = await scrapeCB9ManhattanGCal(board);
          break;
        case "cb11m":
      ...
      if (meetings.length < 2) {
        const manhattanFallbacks = {
          // CB1 now scraped via REST API (cb1m-rest); fallback retained as safety net
          1: [{ name: "Full Board Meeting", weekday: 2, nth: 3, time: "18:00" }],
          // CB9 (Morningside Heights, Hamilton Heights): from generateCB9Meetings schedule
          9: [
            { name: "Full Board Meeting", weekday: 4, nth: 3, time: "18:30" },
            { name: "Executive Committee", weekday: 4, nth: 2, time: "18:30" },
            { name: "Housing, Land Use & Zoning Committee", weekday: 1, nth: 1, time: "18:30" },
            { name: "Transportation Committee", weekday: 2, nth: 1, time: "18:30" },
            { name: "Parks & Environment Committee", weekday: 3, nth: 1, time: "18:30" }
...
/**
 * Scrape Queens CB4 (Corona, Elmhurst) from their CSV calendar.
 * Same format as BK CB9; empty start time = holiday/no meeting.
 */
async function scrapeQB4CSV(board) {
  const CSV_URL = "https://www.nyc.gov/assets/queenscb4/data/queenscb4-calendar.csv";
  const meetings = [], seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);
  ...
  try {
    ...
    for (const line of csv.replace(/\r\n/g, "\n").split("\n").slice(1)) {
      ...
      const [dateStr, startTime, , name] = parseCSVRow(line);
      if (!startTime || !name || name.length < 3) continue;
      const [m, d, y] = dateStr.split("/").map(Number);
      if (!y || !m || !d) continue;
      const meetingDate = new Date(y, m - 1, d);
      if (meetingDate < now || meetingDate > sixMonthsLater) continue;
      ...
      if (!isValidMeetingTitle(cleanName)) continue;
      const dateStr2 = formatDate(y, m - 1, d);
      const key = `${dateStr2}|${cleanName}`;
      if (seen.has(key)) continue; seen.add(key);
      meetings.push(createMeeting(4, `CB4: ${cleanName}`, dateStr2, parseTime(startTime) || "19:00",
        "Queens CB4 Office", board.url, "queens"));
    }
  } catch (err) { console.error("Error scraping QB4 CSV:", err.message); }
  return meetings.filter(Boolean);
}

/**
 * Scrape Queens CB5 (Ridgewood/Glendale/Maspeth) from their custom JS calendar.
 * Format: calEvents[n] = "M/D/YYYY|<span>...<strong>Name</strong><br/>7:30pm...</span>||0|na|"
 * Filter: entries with a time in the HTML are board meetings; "Office Closed" entries are not.
 */
async function scrapeQB5JS(board) {
  const JS_URL = "https://www.nyc.gov/assets/queenscb5/js/calendar_events.js";
  const meetings = [], seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);
  ...
  try {
    const js = await fetchHTML(JS_URL);
    // QB5 uses double-quote delimiters (QB4 used single quotes)
    for (const [, ds, html] of js.matchAll(/calEvents\[calEvents\.length\]\s*=\s*"(\d+\/\d+\/\d+)\|(.+)\|\|0\|na\|";/g)) {
      const rawHtml = html.replace(/\\"/g, '"');
      const timeMatch = rawHtml.match(/(\d{1,2}(?::\d{2})?\s*[ap]m)/i);
      if (!timeMatch) continue;  // no time = holiday/office closed

      const [m, d, y] = ds.split("/").map(Number);
      const meetingDate = new Date(y, m - 1, d);
      if (meetingDate < now || meetingDate > sixMonthsLater) continue;

      const nm = rawHtml.match(/<strong>(.*?)<\/strong>/i);
      if (!nm) continue;
      const name = nm[1].replace(/<[^>]+>/g, "").replace(/&amp;/g, "&").trim();
      if (!name || !isValidMeetingTitle(name)) continue;

      const time = parseTime(timeMatch[1]) || "19:00";
      const dateStr = formatDate(y, m - 1, d);
      const key = `${dateStr}|${name}`;
      if (seen.has(key)) continue; seen.add(key);
      meetings.push(createMeeting(5, `CB5: ${name}`, dateStr, time, "Queens CB5 Office", board.url, "queens"));
    }
  } catch (err) { console.error("Error scraping QB5 JS:", err.message); }
  return meetings.filter(Boolean);
}

/**
 * Scrape Queens CB6 (Forest Hills, Rego Park) from Google Calendar.
 * Filter to board meetings/committees only (the GCal includes community service events).
 */
async function scrapeQB6GCal(board) {
  const BOARD_MEETING_RE = /meeting|committee|briefing|board/i;
  return scrapeGCalICS(["queenscb6secretary@gmail.com"], 6, "queens", board.url, BOARD_MEETING_RE, HOLIDAY_RE);
}

/**
 * Scrape Queens CB13 (Queens Village, Cambria Heights, Laurelton, Rosedale) from Google Calendar.
 * Calendar name "Events and Meetings" — all events are board committee meetings.
 */
async function scrapeQB13GCal(board) {
  return scrapeGCalICS(["seeqcb13@gmail.com"], 13, "queens", board.url, null, HOLIDAY_RE);
}

/**
 * Scrape Queens CB7 (Flushing, Whitestone, College Point) from the static meetings page.
 * Format: "Regular & Public Hearing meeting is scheduled:" in one <p>, then the date in the
 * next <p> ("Monday, June 8, 2026 at 7:00 p.m."), then location.
 * Walks <p> elements in order tracking context.
 */
async function scrapeQB7Static(board) {
  const meetings = [], seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);
  ...
  try {
    ...
    $("h3, p, li").each((_, elem) => {
      ...
      // Detect meeting description lines ("Regular & Public Hearing meeting is scheduled:")
      if (/scheduled|regular.*meeting|public hearing/i.test(text) && !text.match(/\d{4}/)) {
        if (/regular|public hearing/i.test(text)) pendingName = "Full Board Meeting";
        return;
      }

      // Try to find a date with year (Month DD, YYYY)
      const dateMatch = text.match(/(\w+)\s+(\d{1,2}),?\s+(\d{4})/i);
      if (!dateMatch) return;
      const month = parseMonth(dateMatch[1]);
      if (month === -1) return;
      const day = parseInt(dateMatch[2]), year = parseInt(dateMatch[3]);
      const meetingDate = new Date(year, month, day);
      if (meetingDate < now || meetingDate > sixMonthsLater) { pendingName = ""; return; }

      const name = pendingName || "Full Board Meeting";
      pendingName = "";

      const timeMatch = text.match(/(\d{1,2}:\d{2}\s*[ap]\.?m\.?)/i);
      const time = timeMatch ? parseTime(timeMatch[1].replace(/\./g, "")) || "19:00" : "19:00";

      const dateStr = formatDate(year, month, day);
      const key = `${dateStr}|${name}`;
      if (seen.has(key)) return; seen.add(key);
      meetings.push(createMeeting(7, `CB7: ${name}`, dateStr, time, "Queens CB7 Office", board.url, "queens"));
    });
  } catch (err) { console.error("Error scraping QB7 static:", err.message); }
  return meetings.filter(Boolean);
}
...
/**
 * Scrape Queens CB3 via the Tribe Events REST API.
 * The site mixes community events (parades, festivals) with board meetings —
 * filter for events that have "Meeting" in the title.
 */
  ...
  try {
    ...
    while (page <= totalPages && page <= 5) {
      const text = await fetchHTML(
        `https://queenscb3.cityofnewyork.us/wp-json/tribe/events/v1/events?per_page=100&start_date=${startDate}&page=${page}`
      );
      const data = JSON.parse(text);
      totalPages = data.total_pages || 1;
      ...
      for (const event of data.events || []) {
        const title = (event.title || "")
          .replace(/&#038;|&amp;/g, "&").replace(/&#8211;|&ndash;/g, "–").trim();
        if (!title || !/meeting/i.test(title)) continue;
        ...
        meetings.push(createMeeting(3, meetingName, dateStr, `${d.hour}:${d.minutes}`,
          location || "Queens CB3 Office", board.url, "queens"));
      }
      page++;
    }
  } catch (err) {
    ...
    try {
      switch (board.type) {
        ...
        case "queenscb3":
          ...
          break;
        case "queens-cb7-static":
          meetings = await scrapeQB7Static(board);
          break;
        case "queens-cb4-csv":
          meetings = await scrapeQB4CSV(board);
          break;
        case "queens-cb5-js":
          meetings = await scrapeQB5JS(board);
          break;
        case "queens-cb6-gcal":
          meetings = await scrapeQB6GCal(board);
          break;
        case "queens-cb2-gcal":
          meetings = await scrapeQB2MultiGCal(board);
          break;
        case "queens-cb13-gcal":
          meetings = await scrapeQB13GCal(board);
          break;
        case "queens-cb8-pdf":
...
/**
 * Generate recurring meetings for Bronx CB12 (Williamsbridge, Wakefield, Woodlawn).
 * Schedule sourced from the calendar page committee table:
 *   Standing Committee 1 (Land Use/Housing/Economic Dev/Transportation): 2nd Monday
 *   Standing Committee 2 (Public Safety/Parks/Environmental):            1st Thursday
 *   Public Health & Education Committee:                                 3rd Thursday
 *   Full Board Meeting (from bronxBoardSchedules):                       4th Thursday
 */
function generateBX12Meetings(board) {
  const committees = [
    { name: "Land Use, Housing & Transportation Committee",  weekday: 1, nth: 2, time: "18:00" },
    { name: "Public Safety, Parks & Environment Committee",  weekday: 4, nth: 1, time: "18:00" },
    { name: "Public Health & Education Committee",           weekday: 4, nth: 3, time: "18:00" },
    { name: "Full Board Meeting",                            weekday: 4, nth: 4, time: "18:00" },
  ];
  return generateBronxRecurringMeetings(12, committees, "Bronx CB12 Office", board.url);
}

/**
 * Scrape Bronx CB4 (Highbridge, Concourse, Mount Eden) from /calendar/calendar.page.
 * Format: <li>Month DD [&nbsp; padding] Committee Name Meeting</li> — no year.
 * Year is inferred: current year, or next year if the month has already passed.
 */
async function scrapeBX4Calendar(board) {
  const LOCATION = "Bronx CB4 Office";
  const meetings = [], seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);
  ...
  try {
    ...
    $("li").each((_, elem) => {
      // Collapse &nbsp; and extra whitespace, then split date from committee name
      const raw = $(elem).html() || "";
      const text = raw.replace(/&nbsp;| /g, " ").replace(/\s{2,}/g, "  ");
      const clean = text.replace(/<[^>]+>/g, "").replace(/\s{2,}/g, "|").trim();
      // Format: "Month DD|Committee Name"
      const m = clean.match(/^(\w+)\s+(\d{1,2})\s*\|(.+)$/);
      if (!m) return;

      const month = parseMonth(m[1]);
      if (month === -1) return;
      const day = parseInt(m[2]);
      const committeeName = m[3].replace(/&amp;/g, "&").replace(/\s+/g, " ").replace(/\|.*$/, "").trim();
      if (!committeeName || !isValidMeetingTitle(committeeName)) return;
      ...
      const dateStr = formatDate(year, month, day);
      const meetingName = `CB4: ${committeeName}`;
      const key = `${dateStr}|${meetingName}`;
      if (seen.has(key)) return; seen.add(key);
      meetings.push(createMeeting(4, meetingName, dateStr, "18:30", LOCATION, board.url, "bronx"));
    });
  } catch (err) { console.error("Error scraping BX4 calendar:", err.message); }
  return meetings.filter(Boolean);
}

/**
 * Scrape Bronx CB9 (Soundview, Parkchester, Castle Hill) from /calendar/calendar.page.
 * Format: <p>Day, Month DDth, H:MMpm<br/>Committee Name<br/>Location...</p>
 * Multiple meetings can appear within one <p>, separated by <br/>&nbsp;<br/>.
 * No year — inferred from current date.
 */
async function scrapeBX9Calendar(board) {
  const DEFAULT_LOCATION = "Bronx CB9 Office, 1967 Turnbull Ave. Suite 201, Bronx, NY";
  const meetings = [], seen = new Set();
  const now = new Date(); now.setHours(0, 0, 0, 0);
  const sixMonthsLater = new Date(); sixMonthsLater.setMonth(sixMonthsLater.getMonth() + 6);

  // DATE_LINE: "Monday, June 1st, 7:00pm" or "Thursday, June 4th, 7:00pm"
  const DATE_LINE = /^(?:Monday|Tuesday|Wednesday|Thursday|Friday|Saturday|Sunday),\s+(\w+)\s+(\d{1,2})(?:st|nd|rd|th)?,\s*(\d{1,2}:\d{2}\s*[ap]m)/i;
  ...
  try {
    ...
    $("p").each((_, elem) => {
      // Split paragraph on <br> to get individual lines
      const pHtml = $(elem).html() || "";
      ...
      while (i < lines.length) {
        const dateLine = lines[i].match(DATE_LINE);
        if (!dateLine) { i++; continue; }

        const month = parseMonth(dateLine[1]);
        if (month === -1) { i++; continue; }
        const day = parseInt(dateLine[2]);
        const time = parseTime(dateLine[3]) || "19:00";

        // Next non-empty line is the committee name — must look like a board meeting, not a venue
        const committeeName = lines[i + 1] ? lines[i + 1].trim() : "";
        const isBoardMeeting = /committee|board|meeting|commission|council|public safety|land use|zoning|planning|parks|transport|health|housing|education|youth|sanitation|environment/i.test(committeeName);
        if (!committeeName || !isValidMeetingTitle(committeeName) || !isBoardMeeting) { i++; continue; }

        // Location: line after committee (if it looks like an address)
        const locationLine = lines[i + 2] || "";
        const location = locationLine && !/^\d{4}/.test(locationLine) ? locationLine : DEFAULT_LOCATION;
        ...
        if (meetingDate >= now && meetingDate <= sixMonthsLater) {
          const dateStr = formatDate(year, month, day);
          const meetingName = `CB9: ${committeeName}`;
          const key = `${dateStr}|${meetingName}`;
          if (!seen.has(key)) {
            seen.add(key);
            meetings.push(createMeeting(9, meetingName, dateStr, time, location || DEFAULT_LOCATION, board.url, "bronx"));
          }
        }
        i += 2;
      }
    });
  } catch (err) { console.error("Error scraping BX9 calendar:", err.message); }
  return meetings.filter(Boolean);
}
    ...
    try {
      switch (board.type) {
        ...
        case "cbbronx":
          ...
          break;
        case "bronx-cb4-calendar":
          meetings = await scrapeBX4Calendar(board);
          break;
        case "bronx-cb9-calendar":
          meetings = await scrapeBX9Calendar(board);
          break;
        case "bronx-cb12-generated":
          meetings = generateBX12Meetings(board);
          break;
        case "bronx-cb10-pdf":
