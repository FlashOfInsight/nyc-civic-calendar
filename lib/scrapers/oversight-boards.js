...
const { fetchHTML, parseMonth, formatDate, isInRange } = require("../scraper-utils");
...
async function scrapeLPC() {
  ...
  try {
    ...
    // If we found actual dates from the LPC page, use only those
    // Otherwise fall back to generating all Tuesdays
    if (foundDates.size > 0) {
      console.log(`LPC: using ${foundDates.size} actual dates from page`);
      ...
      console.warn("LPC: no dates parsed from page, falling back to all Tuesdays");
