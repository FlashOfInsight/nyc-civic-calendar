...
function generateEvent(meeting) {
  ...
  // UID - unique identifier (v2 suffix forces calendar apps to treat as new events)
  ...
  // Start time
  if (meeting.time) {
    // Use floating local time format (no Z suffix, no TZID)
    // This is interpreted as the viewer's local time, which works for NYC civic meetings
    const startTimeFormatted = formatLocalDateTime(meeting.date, meeting.time);
    lines.push(`DTSTART:${startTimeFormatted}`);

    // End time
    if (meeting.endTime) {
      const endTimeFormatted = formatLocalDateTime(meeting.date, meeting.endTime);
      lines.push(`DTEND:${endTimeFormatted}`);
    } else {
  ...
  // Description
  if (meeting.description || meeting.url || meeting.agendaUrl) {
    let desc = meeting.description || "";
    if (meeting.agendaUrl) {
      desc += desc ? `\n\nAgenda: ${meeting.agendaUrl}` : `Agenda: ${meeting.agendaUrl}`;
    }
    if (meeting.url) {
      desc += desc ? `\n\nMore info: ${meeting.url}` : `More info: ${meeting.url}`;
    }
