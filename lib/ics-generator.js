...
// Migration notice prepended to every event's description when the caller
// passes { migrationPrefix: true }. Only the OLD (Vercel) endpoint opts in;
// the new Cloudflare endpoint serves clean events. Delete this block and
// the migrationPrefix option after the cutoff date.
const MIGRATION_NOTICE_TEXT = [
  "[NYC CIVIC CALENDAR IS MOVING]",
  "Re-subscribe at https://nycciviccalendar.com before June 30, 2026 — this URL stops updating after that date.",
  "IMPORTANT: Remove this subscription before adding the new one, or you'll see duplicate events in your calendar.",
  "Why: proactive migration off Vercel following their security incident. Our data was not affected.",
  "-----"
].join("\n\n");
 ...
 * @param {boolean} [options.migrationPrefix] - Prepend MIGRATION_NOTICE_TEXT to each event's DESCRIPTION. Used only by the legacy Vercel endpoint during the grace period.
 * @returns {string} - ICS formatted string
 */
function generateICS(meetings, calendarName = "NYC Civic Meetings", options = {}) {
  const lines = [
    "BEGIN:VCALENDAR",
    "VERSION:2.0",
  ...
  for (const meeting of meetings) {
    lines.push(...generateEvent(meeting, options));
  }
 ...
 * @param {Object} [options]
 * @param {boolean} [options.migrationPrefix] - Prepend MIGRATION_NOTICE_TEXT to DESCRIPTION
 * @returns {Array} - Array of ICS lines
 */
function generateEvent(meeting, options = {}) {
  ...
  if (options.migrationPrefix) descriptionParts.push(MIGRATION_NOTICE_TEXT);
